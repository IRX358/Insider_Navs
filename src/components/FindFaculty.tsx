...
export const FindFaculty: React.FC<FindFacultyProps> = ({ onRouteToFaculty }) => {
  ...
  const filteredFaculty = (selectedSchool !== null || selectedRole !== null || showFavoritesOnly) 
    ? allFaculty.filter(faculty => {
        ...
        const favoriteMatch = showFavoritesOnly ? favoriteIds.includes(faculty.id) : true;
        return schoolMatch && roleMatch && favoriteMatch;
      })
    : [];
      ...
      <button
        ...
        className={`
          ...
          ${showFavoritesOnly
            ...
            : 'glass-panel text-gray-400 hover:text-yellow-400 border border-transparent hover:border-yellow-500/30'
          }
        `}
      >
        <Star size={18} className={showFavoritesOnly ? 'fill-yellow-400' : ''} />
        {showFavoritesOnly ? 'Showing Favorites Only' : 'Show My Favorites'}
      </button>
